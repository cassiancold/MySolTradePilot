...
async def get_balance(user_id: int):
    """Get real SOL balance from the user's wallet via Solana RPC."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "Use the buttons below to interact quickly or menu commands.\n\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "- Balance: Shows your SOL and MEME token balances.\n"
        "- Buy MEME: Buy MEME tokens with SOL.\n"
        "- Sell MEME: Sell your MEME tokens for SOL.\n"
...
async def buy_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sol_balance <= 0:
        ...
        await update.message.reply_text(
            f"⚠️ You have no SOL in your wallet!\nFund your wallet first to buy MEME tokens.\n\n🏦 Wallet Address:\n{pub_key}"
        )
        return
...
async def sell_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if tokens <= 0:
        await update.message.reply_text(
            "⚠️ You have no MEME tokens to sell!\nBuy some MEME first, then you can sell later."
        )
        return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "create_wallet":
        ...
        await query.message.reply_text(f"✅ Wallet created!\n🏦 Address: {pub}\nKeep your private key safe!", reply_markup=get_start_keyboard())
    elif query.data == "sol_address":
        if user_id not in user_wallets:
            await query.message.reply_text("Create a wallet first!", reply_markup=get_start_keyboard())
        else:
        ...
        if user_id not in user_wallets:
            ...
            sol_balance = await get_balance(user_id)
            meme_balance = await get_tokens(user_id)
            await query.message.reply_text(f"💰 Balance:\nSOL: {sol_balance:.6f}\nMEME: {meme_balance}", reply_markup=get_start_keyboard())
